...
class Dreamless_Downloader:
    ...
    if os.path.exists(filepath):
        ...
        if self.remote_size > 0 and local_size >= self.remote_size * 0.99:
            print(
                f"[Dreamless] Using existing file: {self.name} ({self.format_size(local_size)})"
            )
            return filepath
    ...
    try:
        req = urllib.request.Request(
            self.download_url, headers=headers, method="HEAD"
        )
        with urllib.request.urlopen(req, timeout=self.timeout) as resp:
            url_final = resp.geturl()
            total_size = int(resp.headers.get("Content-Length", self.remote_size))
    ...
    def download_part(part_id, start_byte, end_byte):
        ...
        part_req = urllib.request.Request(final_url)
        part_req.add_header("User-Agent", "Mozilla/5.0")
        part_req.add_header("Range", f"bytes={start}-{end}")
