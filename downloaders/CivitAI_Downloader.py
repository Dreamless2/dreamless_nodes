...
class CivitAI_Downloader:
    ...
    def __init__(
        ...
        if token and token.strip():
            ...
            self.token = get_api_key("civitai")
    ...
    def details(self):
        ...
        if response.status_code == 200:
            ...
            if model_type not in self.valid_types:
                raise Exception(
                    f"{ERR_PREFIX}The model you requested is not a valid `{', '.join(self.valid_types)}`. Aborting!"
                )
            ...
            raise Exception(
                f"{ERR_PREFIX}No cached model or model data found, and unable to reach CivitAI! Response Code: {response.status_code}\n Please try again later."
            )
    ...
    def download(self):
        ...
        def download_chunk(
            ...
            while retries <= max_retries:
                try:
                    headers = {
                        "Range": f"bytes={start_byte + downloaded_bytes}-{end_byte}"
                    }
                    ...
                    if response.status_code == 206:
                        with open(file_path, "r+b") as file:
                            if retries > 0:
                                print(
                                    f"{MSG_PREFIX}Chunk {chunk_id} re-established in {retry_delay}s"
                                )
                                total_pbar.update()
                            ...
                            comfy_pbar.set_postfix_str(
                                f"Chunk {chunk_id}: {downloaded_bytes} bytes downloaded"
                            )
                    else:
                        raise Exception(
                    ...
                    if retries > max_retries:
                        print(
                            f"{ERR_PREFIX}Chunk {chunk_id} failed to download after {max_retries} retries."
                        )
                        break
        ...
        def get_total_file_size(url):
            ...
            response = requests.get(
                url, headers={"Range": "bytes=0-999999999"}, stream=True
            )
        ...
        if model_name:
            ...
            if model_path:
                model_sha256 = CivitAI_Downloader.calculate_sha256(model_path)
                if self.name == model_name:
                    print(
                        f"{MSG_PREFIX}Loading {self.type}: {self.name} (https://civitai.com/models/{self.model_id}/?modelVersionId={self.version})"
                    )
        ...
        if os.path.exists(save_path):
            ...
            existing_sha256 = CivitAI_Downloader.calculate_sha256(save_path)
        ...
        if response.status_code != requests.codes.ok:
            raise Exception(
                f"{ERR_PREFIX}Failed to download {self.type} file from CivitAI. Status code: {response.status_code}"
            )
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            total_pbar.close()
        model_sha256 = CivitAI_Downloader.calculate_sha256(save_path)
                    ...
                    if version_id:
                        for file_details in version.get("files", []):
                            if (
                                ...
                                print(
                                    f"{MSG_PREFIX}Loading {model_type}: {os.path.basename(file_path)} (https://civitai.com/models/{model_id}/?modelVersionId={version_id})"
                                )
                                print(f"{MSG_PREFIX}{model_type} Sha256: {hash_value}")
                                return (model_id, version_id, file_details)

        api = f"{CivitAI_Downloader.api}/model-versions/by-hash/{hash_value}"
            ...
            if model_details:
                ...
                for file_details in model_versions:
                    ...
                    if hashes and hash_value in hashes.values():
                        ...
                        print(f"{MSG_PREFIX}{model_type} Sha256: {hash_value}")
                        file_details.update(
                            {"model_type": model_type, "trained_words": trained_words}
                        )
                        CivitAI_Downloader.push_download_history(
                            model_id, model_type, file_details
                        )
                        return (model_id, version_id, file_details)
                    else:
                        if CivitAI_Downloader.warning:
                            print(
                                f"{WARN_PREFIX}Unable to determine `{os.path.basename(file_path)}` source on CivitAI."
                            )
        else:
            if CivitAI_Downloader.warning:
                print(
                    ...
                    for version_details in model_versions:
                        if version_details.get("id") == file_details.get("id"):
                            ...
                            for file_info in files:
                                if file_info.get("downloadUrl") == file_details.get(
                                    "downloadUrl"
                                ):
                                    return
