...
class Dreamless_LORA_Loader_CivitAI:
    ...
    def load_lora(
        ...
        if lora_name == "none":
            ...
            if not lora_id:
                raise ValueError(
                    f"{MSG_PREFIX}Please provide a valid LoRA/LyCORIS AIR or select a local file."
                )
            ...
            print(f"{MSG_PREFIX}Loading LoRA from disk: {lora_name}: model: {strength_model}, clip: ")
